in_file =open("sample_input.txt","r") #open input file to read input

out_file = open("sample_output.txt","w+") #open output file to write output
goodies={}
out_list=[]
...
single_prices.sort(reverse=True) #sorted list in decsending order to get prices to be distributed in order
print(single_prices)

num_of_employees=int(input("Enter number of employees: ")) #taking inputs
...
diff=max(choosen_prices)-min(choosen_prices) #differnce between high and low price
print("diff",diff)
...
out_file.write("Here the goodies that are selected for distribution are: ")
out_file.write("\n\n")
